...
class VolumeManagerBlockTest(VolumeManagerTestBase):
    ...
    def testBlock(self):
        ...
        self.assertSetEqual(block.NonStosSectionNumbers, frozenset([]), "Non stos section numbers should initialize to empty list")
        block.MarkSectionsAsDamaged(test_data)
        self.assertSetEqual(block.NonStosSectionNumbers, test_data, "Damaged sections should be reflected in the block node")
        
        block.MarkSectionsAsUndamaged(even_data)
        self.assertSetEqual(block.NonStosSectionNumbers, odd_data, "Odd sections should remain after removing even numbers")
        ...
        self.assertSetEqual(loaded_block.NonStosSectionNumbers, odd_data, "Odd sections should remain after removing even numbers and save+load")
        ...
        self.assertSetEqual(loaded_block.NonStosSectionNumbers, test_data, "All sections should be marked after adding even numbers")
        ...
        (added_block, loaded_block) = self.VolumeObj.UpdateOrAddChild(block)
        self.assertFalse(added_block, "Block should exist after running pipeline")
        self.assertSetEqual(loaded_block.NonStosSectionNumbers, even_data, "Even sections should be marked after removing odd numbers")
