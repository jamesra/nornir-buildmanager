...
class VolumeEntry(object):
    ...
    def xpath(self):
        ...
        if not self.AttributeName is None:
            x = VolumeEntry.xpathtemplate % {'xpath': self.XPath, 'AttribName': self.AttributeName,
                                             'AttribValue': self.AttributeValue}
...
class NornirBuildTestBase(testbase.TestBase):
    ...
    def CheckTransformInputs(self, transformNode):
        ...
        self.assertTrue('InputTransform' in transformNode.attrib,
                        "Missing InputTranform attribute:\n" + transformNode.ToElementString())
    ...
    def RunSetPruneCutoff(self, Value, Section, Channels, Filters):
        ...
        buildArgs = self._CreateBuildArgs('SetPruneCutoff', '-Sections', Section, '-Channels', Channels, '-Filters',
        ...
        for fnode in Filters:
            ...
            self.assertEqual(pNode.UserRequestedCutoff, float(Value),
                             "Value on prune node should match the passed value")
    ...
    def RunSetContrast(self, MinValue, MaxValue, GammaValue, Section, Channels, Filters):
        ...
        for fnode in Filters:
            ...
            if math.isnan(float(MinValue)):
                ...
                self.assertEqual(ahNode.UserRequestedMinIntensityCutoff, float(MinValue),
                                 "Min value should match the passed value")
            ...
            if math.isnan(float(MaxValue)):
                ...
                self.assertEqual(ahNode.UserRequestedMaxIntensityCutoff, float(MaxValue),
                                 "Max value should match the passed value")

            if math.isnan(float(GammaValue)):
                self.assertIsNone(ahNode.UserRequestedGamma, "Gamma value should match the passed value")
            else:
                self.assertEqual(ahNode.UserRequestedGamma, float(GammaValue),
                                 "Gamma value should match the passed value")
    ...
    def RunShadingCorrection(self, ChannelPattern: str, CorrectionType=None, FilterPattern=None):
        ...
        buildArgs = self._CreateBuildArgs('ShadeCorrect', '-Channels', ChannelPattern, '-Filters', FilterPattern,
                                          '-OutputFilter', 'ShadingCorrected', '-Correction', CorrectionType)
        volumeNode = self.RunBuild(buildArgs)
    ...
    def RunHistogram(self, Filter=None, Downsample: int = 4, Transform=None):
        ...
        # Adjust Contrast
        buildArgs = self._CreateBuildArgs('Histogram', '-Filters', Filter, '-Downsample', str(Downsample),
    ...
    def RunAdjustContrast(self, Sections=None, Filter=None, Gamma: float | None = None, Transform=None):
        ...
        # Adjust Contrast
        if Sections is None:
            buildArgs = self._CreateBuildArgs('AdjustContrast', '-InputFilter', Filter, '-OutputFilter', 'Leveled',
                                              '-InputTransform', Transform)
        else:
            buildArgs = self._CreateBuildArgs('AdjustContrast', '-Sections', str(Sections), '-InputFilter', Filter,
                                              '-OutputFilter', 'Leveled', '-InputTransform', Transform)
    ...
    def _VerifyInputTransformIsCorrect(self, InputTransformChecksumNode, input_transform_name: str):
        ...
        self.assertEqual(InputTransformChecksumNode.InputTransformChecksum, TransformNode.Checksum,
                         "Checksum does not match the transform")
        self.assertEqual(InputTransformChecksumNode.InputTransformType, TransformNode.Type,
                         "Type does not match the transform")
        self.assertEqual(InputTransformChecksumNode.InputTransformCropBox, TransformNode.CropBox,
                         "CropBox does not match the transform")

        self.assertTrue(InputTransformChecksumNode.IsInputTransformMatched(TransformNode),
                        "IsInputTransformMatched should return true when the earlier tests in this function have passed")

    def _VerifyImageSetMatchesTransform(self, image_set_node, transform_name):

        self.assertEqual(image_set_node.InputTransform, transform_name,
                         "InputTransform for ImageSet does not match transform used for assemble")
    ...
    def RunAssemble(self, Channels=None, Filter=None, TransformName=None, Levels=8):
        ...
        if not Channels is None:
            buildArgs = self._CreateBuildArgs('Assemble', '-Channels', Channels, '-Transform', TransformName,
                                              '-Filters', Filter, '-Downsample', LevelsStr, '-NoInterlace')
        else:
            buildArgs = self._CreateBuildArgs('Assemble', '-Transform', TransformName, '-Filters', Filter,
                                              '-Downsample', LevelsStr, '-NoInterlace')
    ...
    def RunAssembleTiles(self, Channels=None, Filter=None, TransformName=None, Levels=2, Shape=[512, 512],
        ...
        buildArgs = []
        if not Channels is None:
            buildArgs = self._CreateBuildArgs('AssembleTiles', '-Channels', Channels, '-Transform', TransformName,
                                              '-Filters', Filter, '-HighestDownsample', str(Levels), '-Shape', ShapeStr)
        else:
            buildArgs = self._CreateBuildArgs('AssembleTiles', '-Transform', TransformName, '-Filters', Filter,
                                              '-HighestDownsample', str(Levels), '-Shape', ShapeStr)
    ...
    def RunAlignSections(self, Channels, Filters, Levels, Angles=None, Center=None, UseMasks=True, NoFlipCheck=False):
        ...
        if not Center is None:
            buildArgs = self._CreateBuildArgs('AlignSections', '-NumAdjacentSections', '1', '-Filters', Filters,
                                              '-Downsample', str(Levels), '-Channels', Channels, '-Center', str(Center))
        ...
        self.VerifyStosTransformPipelineSharedTests(volumeNode, stos_map_name='PotentialRegistrationChain',
                                                    stos_group_name='%s%d' % ('StosBrute', Levels),
                                                    MasksRequired='-UseMasks' in buildArgs, buildArgs=buildArgs)
    ...
    def RunAssembleStosOverlays(self, Group, Downsample, StosMap):
        ...
        buildArgs = self._CreateBuildArgs('AssembleStosOverlays', '-StosGroup', Group, '-Downsample', str(Downsample),
                                          '-StosMap', StosMap)
        volumeNode = self.RunBuild(buildArgs)
    ...
    def RunSelectBestRegistrationChain(self, Group, Downsample, InputStosMap, OutputStosMap):
        ...
        buildArgs = self._CreateBuildArgs('SelectBestRegistrationChain', '-StosGroup', Group, '-Downsample',
                                          str(Downsample), '-InputStosMap', InputStosMap, '-OutputStosMap',
                                          OutputStosMap)
        volumeNode = self.RunBuild(buildArgs)
        ...
        self.assertEqual(InputStosMap.CenterSection, OutputStosMap.CenterSection,
                         "Center section should not change when selecting the best registration chain")
    ...
    def RunRefineSectionAlignment(self, InputGroup, InputLevel, OutputGroup, OutputLevel, Filter, UseMasks=True):
        ...
        self.VerifyStosTransformPipelineSharedTests(volumeNode=volumeNode,
                                                    stos_group_name='%s%d' % (OutputGroup, OutputLevel),
                                                    stos_map_name='FinalStosMap', MasksRequired=UseMasks,
                                                    buildArgs=buildArgs)
    ...
    def RunScaleVolumeTransforms(self, InputGroup, InputLevel, OutputLevel=1):
        # Build Mosaics
        buildArgs = self._CreateBuildArgs('ScaleVolumeTransforms', '-InputGroup', InputGroup, '-InputDownsample',
                                          str(int(InputLevel)), '-OutputDownsample', str(int(OutputLevel)))
        volumeNode = self.RunBuild(buildArgs)
        ...
        self.VerifyStosTransformPipelineSharedTests(volumeNode=volumeNode,
                                                    stos_group_name='%s%d' % (InputGroup, OutputLevel),
                                                    stos_map_name='FinalStosMap', MasksRequired=MasksRequired,
                                                    buildArgs=buildArgs)
        return volumeNode
    ...
    def RunSliceToVolume(self, Level=1):
        ...
        buildArgs = self._CreateBuildArgs('SliceToVolume', '-Downsample', str(Level), '-InputGroup', 'Grid',
        ...
        self.VerifyStosTransformPipelineSharedTests(volumeNode=volumeNode, stos_group_name='%s%d' % (group_name, Level),
                                                    stos_map_name='FinalStosMap', MasksRequired=MasksRequired,
                                                    buildArgs=buildArgs)
        return volumeNode
    ...
    def RunMosaicToVolume(self):
        # Build Mosaics
        buildArgs = self._CreateBuildArgs('MosaicToVolume', '-InputTransform', 'Grid', '-OutputTransform',
                                          'ChannelToVolume', '-Channels', '*')
        volumeNode = self.RunBuild(buildArgs)
    ...
    def RunCreateVikingXML(self, OutputFile, StosGroup=None, StosMap=None):
        ...
        self.assertTrue(os.path.exists(os.path.join(volumeNode.FullPath, OutputFile + ".VikingXML")),
                        "No vikingxml file created")
    ...
    def RemoveAndRegenerateTile(self, RegenFunction, RegenKwargs, section_number: int, channel: str = 'TEM',
        ...
        self.assertTrue(os.path.exists(removedTileFullPath),
    ...
    def CopyManualStosFiles(self, ManualStosFullPath, StosGroupName):
        ...
        for f in glob.glob(os.path.join(ManualStosFullPath, '*.stos')):
            ...
            self.assertIsNotNone(stosTransform,
                                 "Stos transform file that is overriden by manual files does not exist: %s" % stosFilePath)
