...
class PruneTest(setup_pipeline.ImportOnlySetup):
    ...
    def testPruneScoreGeneration(self):
        ...
        OutputFilterNode = PruneObj.CalculatePruneScores({}, self.FilterNode, 2, self.StageTransformNode,
                                                         OutputFile='PruneData', Logger=self.Logger)
        ...
        OutputFilterNode = PruneObj.CalculatePruneScores({}, self.FilterNode, 2, self.StageTransformNode,
                                                         OutputFile=PruneFileFullPath, Logger=self.Logger)
        self.assertIsNone(OutputFilterNode)

        # OK, try to see what .mosaic we get with the prune scores
        (TransformParent, PruneParent) = PruneObj.PruneMosaic({}, PruneNode=PruneNode,
                                                              TransformNode=self.StageTransformNode,
                                                              OutputTransformName='Prune', Logger=self.Logger)
        ...
        Output = PruneObj.PruneMosaic({}, PruneNode=PruneNode, TransformNode=self.StageTransformNode,
                                      OutputTransformName='Prune', Logger=self.Logger)
